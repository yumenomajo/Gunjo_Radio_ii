...
$.ajax({
  ...
  data: {
    key: 'HQTBZ-5LN6L-MOEPN-MHUMA-3QATT-JGFPD',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(113.53031500, 22.34767500, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
  ...
  let timeChange;
  let date = new Date();
  if (date.getHours() >= 5 && date.getHours() < 11) timeChange = "<span>早上好</span>，一日之计在于晨！";
  else if (date.getHours() >= 11 && date.getHours() < 13) timeChange = "<span>中午好</span>，该摸鱼吃午饭了。";
  else if (date.getHours() >= 13 && date.getHours() < 15) timeChange = "<span>下午好</span>，懒懒地睡个午觉吧！";
  else if (date.getHours() >= 15 && date.getHours() < 16) timeChange = "<span>三点几啦</span>，一起饮茶呀！";
  else if (date.getHours() >= 16 && date.getHours() < 19) timeChange = "<span>孤落时辰到了，你的名字是什么呢？</span>";
  else if (date.getHours() >= 19 && date.getHours() < 24) timeChange = "<span>晚上好</span>，无论身处何时何地，都不要忘记自己头顶那块美丽的星空。";
  else timeChange = "深邃的夜，微弱的光，友人A在做什么呢？";
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Yumenomajo🧙 %c 你正在访问群青广播II📡",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| 群青广播II📡") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('群青广播II📡的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
document.addEventListener('visibilitychange', function () {
  if (document.hidden) {
    //离开当前页面时标签显示内容
    document.title = '🌙生存便是寂寞';
    clearTimeout(titleTime);
  } else {
    //返回当前页面时标签显示内容
    document.title = '🌻人啊，幸福吧~';
    //两秒后变回正常标题
...
function createtime() {
  ...
  var start = new Date("11/13/2024 17:09:22"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(23400000000 + ((now - start) / 1000) * 17); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("11/13/2024 17:09:22");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? ` <div style="font-size:13px;font-weight:bold">本站运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> </div>`
      : ` <div style="font-size:13px;font-weight:bold">本站运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> </div>`),
